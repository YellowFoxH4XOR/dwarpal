...
## Purpose
TBD - created by archiving change m3-launch. Update Purpose after archive.
## Requirements
### Requirement: goreleaser-driven release matrix
Releases SHALL be produced by a single `.goreleaser.yaml` covering darwin/linux/windows on amd64/arm64, publishing static binaries (`CGO_ENABLED=0`; the AST tier uses the stdlib `go/parser`, so no cgo is required), and checksums, attached to a GitHub Release on each tag push.
