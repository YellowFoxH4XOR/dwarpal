...
### Requirement: Provenance attached as a git note
On a successful `dwarpal check`, Dwarpal SHALL attach detected provenance (`agent`, `detected_via`, `timestamp`) as a git note under a dedicated ref, without amending the commit.

#### Scenario: Provenance note recorded
- **WHEN** `dwarpal check` runs against a commit detected as authored by `claude-code` via trailer
- **THEN** a git note under the dwarpal provenance ref is created for that commit containing `agent: claude-code` and `detected_via: trailer`, and the commit SHA is unchanged
