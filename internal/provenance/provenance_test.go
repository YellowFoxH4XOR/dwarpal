...
// Configurable heuristics are the fourth, weakest signal (checklist #16).
func TestDetect_Heuristics(t *testing.T) {
	t.Setenv(EnvVar, "")
	d := New(nil, nil).WithHeuristics([]string{`(?i)copilot`, `bot/`})
	if got := d.Detect("bot/refactor", ""); !got.IsAgent || got.Source != SourceHeuristic {
		t.Fatalf("branch heuristic failed: %+v", got)
	}
	if got := d.Detect("main", "chore: sync\n\nGenerated by Copilot agent"); !got.IsAgent || got.Source != SourceHeuristic {
		t.Fatalf("message heuristic failed: %+v", got)
	}
	if got := d.Detect("main", "regular human commit"); got.IsAgent {
		t.Fatalf("no-signal commit misflagged: %+v", got)
	}
	// Invalid pattern is skipped, not fatal.
	d2 := New(nil, nil).WithHeuristics([]string{`([`})
	if got := d2.Detect("anything", ""); got.IsAgent {
		t.Fatalf("invalid pattern must match nothing: %+v", got)
	}
}
